...
function extractFields(nalType, data) {
    ...
    if (nalType === 32) {
        ...
        fields.push({ name: "sps_conf_win_left_offset", value: ((data[18] & 0x03) << 16) | ((data[19] & 0xFF) << 8) | data[20] });
        fields.push({ name: "sps_conf_win_right_offset", value: ((data[21] & 0x03) << 16) | ((data[22] & 0xFF) << 8) | data[23] });
        fields.push({ name: "sps_conf_win_top_offset", value: ((data[24] & 0x03) << 16) | ((data[25] & 0xFF) << 8) | data[26] });
        fields.push({ name: "sps_conf_win_bottom_offset", value: ((data[27] & 0x03) << 16) | ((data[28] & 0xFF) << 8) | data[29] });
        fields.push({ name: "sps_bit_depth_luma_minus8", value: (data[8] >> 4) & 0x03 });
        fields.push({ name: "sps_bit_depth_chroma_minus8", value: data[8] & 0x03 });
